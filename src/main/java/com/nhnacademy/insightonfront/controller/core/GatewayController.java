 ...
 * <p>groupId는 세션에서 읽는다 — 한 유저는 그룹 하나에만 속해서 URL에 실을 필요가 없다.
 */
@Controller
...
public class GatewayController {
    ...
    public String detail(@SessionAttribute(value = "userId", required = false) Long userId,
        ...
        GatewayResponse gateway = findGateway(userId, groupId);
    ...
    @PostMapping
    @ResponseBody
    public GatewayResponse create(@SessionAttribute(value = "userId", required = false) Long userId,
                                   @SessionAttribute(value = "groupId", required = false) Long groupId,
                                   @RequestBody GatewayForm form) {
        return gatewayClient.create(userId,
    ...
    public void update(@SessionAttribute(value = "userId", required = false) Long userId,
                        ...
                        @RequestBody GatewayForm form) {
        gatewayClient.update(userId, gatewayId,
    ...
    public void delete(@SessionAttribute(value = "userId", required = false) Long userId,
                        @PathVariable("gateway-id") Long gatewayId) {
        gatewayClient.delete(userId, gatewayId);
    }

    private GatewayResponse findGateway(Long userId, Long groupId) {
        try {
